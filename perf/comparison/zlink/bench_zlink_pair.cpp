...
void run_pair(const std::string& transport, size_t msg_size, int msg_count) {
    ...
    int rc = zlink_bind(s_bind, endpoint.c_str());
    if (rc != 0) { std::cerr << "bind failed: " << zlink_strerror(errno) << std::endl; return; }
    rc = zlink_connect(s_conn, endpoint.c_str());
    if (rc != 0) { std::cerr << "connect failed: " << zlink_strerror(errno) << std::endl; return; }
