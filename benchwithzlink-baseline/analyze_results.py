...
def generate_markdown(data):
    ...
    md = f"""# Windows Benchmark Results - zlink vs libzlink (10x runs with CPU affinity)
...
- **libzlink (reference)**: v4.3.5 with CURVE/libsodium enabled
...
| Pattern | libzlink (msg/s) | zlink (msg/s) | Difference | libzlink (μs) | zlink (μs) | Difference |
|---------|---------------|--------------|------------|-------------|-----------|------------|
"""
    ...
    for pattern in patterns_64:
        libzlink_thr_key = ('libzlink', pattern, 64, 'throughput')
        zlink_thr_key = ('zlink', pattern, 64, 'throughput')
        libzlink_lat_key = ('libzlink', pattern, 64, 'latency')
        zlink_lat_key = ('zlink', pattern, 64, 'latency')

        if libzlink_thr_key in data and zlink_thr_key in data:
            libzlink_thr = calculate_stats(data[libzlink_thr_key]['values'])
            zlink_thr = calculate_stats(data[zlink_thr_key]['values'])
            libzlink_lat = calculate_stats(data[libzlink_lat_key]['values'])
            zlink_lat = calculate_stats(data[zlink_lat_key]['values'])

            thr_diff = ((zlink_thr['mean'] - libzlink_thr['mean']) / libzlink_thr['mean']) * 100
            lat_diff = ((zlink_lat['mean'] - libzlink_lat['mean']) / libzlink_lat['mean']) * 100

            md += f"| **{pattern.replace('_', '/')}** | "
            md += f"{format_number(libzlink_thr['mean'])} ± {format_number(libzlink_thr['stdev'])} | "
            md += f"{format_number(zlink_thr['mean'])} ± {format_number(zlink_thr['stdev'])} | "
            md += f"{thr_diff:+.1f}% | "
            md += f"{libzlink_lat['mean']:.2f} ± {libzlink_lat['stdev']:.2f} | "
            md += f"{zlink_lat['mean']:.2f} ± {zlink_lat['stdev']:.2f} | "
            md += f"{lat_diff:+.1f}% |\n"

    md += "\n### 1024-byte Messages\n\n"
    md += "| Pattern | libzlink (msg/s) | zlink (msg/s) | Difference | libzlink (μs) | zlink (μs) | Difference |\n"
    md += "|---------|---------------|--------------|------------|-------------|-----------|------------|\n"

    patterns_1024 = ['PAIR', 'PUBSUB']

    for pattern in patterns_1024:
        libzlink_thr_key = ('libzlink', pattern, 1024, 'throughput')
        zlink_thr_key = ('zlink', pattern, 1024, 'throughput')
        libzlink_lat_key = ('libzlink', pattern, 1024, 'latency')
    ...
    md += "- Draft API: OFF\n"
    md += "- Limited socket types (PAIR, PUB/SUB, XPUB/XSUB, DEALER/ROUTER)\n\n"
