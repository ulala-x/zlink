# zlink Performance Baseline: libzlink 대체 가능성 평가

## 1. 서론

### 1.1 zlink란 무엇인가?

zlink는 libzlink 4.3.5를 기반으로 한 **간소화된 대체재**입니다. 레거시 기능을 제거하고 현대적인 기술 스택으로 재구성하여, 더 작고 유지보수가 용이한 메시징 라이브러리를 목표로 합니다.
...
> **"zlink가 libzlink를 대체할 때 성능 희생이 발생하는가?"**
...
| 패턴 | libzlink (msg/s) | zlink (msg/s) | 차이 | 판정 |
|------|---------------|--------------|------|------|
...
| 사용 사례 | libzlink 대체 가능? | 성능 영향 | 조건/제약 |
|----------|------------------|----------|----------|
...
Q1: 128KB 이상 대용량 메시지가 주력인가?
    |
    +-- YES --> 주의 필요. libzlink 유지 또는 메시지 분할 적용
    |
    +-- NO --> Q2로 진행
...
| **기존 libzlink 마이그레이션** | 조건부 권장 (제거된 기능 확인 필요) |
| **이벤트 기반 시스템** | **zlink 적극 권장** (+41% 성능 향상) |
| **대용량 데이터 전송** | libzlink 유지 또는 청크 분할 적용 |
...
**목표**: DEALER/ROUTER inproc 성능 libzlink 동등화
...
```bash
# Linux - zlink vs libzlink 비교
cd benchwithzlink-baseline
./run_benchmarks.sh --with-libzlink --runs 3
...
| 파일 | 설명 |
|------|------|
| `benchwithzlink-baseline/zlink_results.csv` | zlink 원본 데이터 |
| `benchwithzlink-baseline/results_10x.csv` | zlink vs libzlink 비교 |
| `benchwithzlink-baseline/BENCHMARK_RESULTS.md` | 상세 결과 문서 |
...
| 버전 | 날짜 | 변경 내용 |
|------|------|----------|
| 2.0 | 2026-01-14 | libzlink 대체 가능성 평가 관점으로 재작성 |
| 1.0 | 2026-01-14 | 초기 베이스라인 문서 작성 |

---

*이 문서는 zlink가 libzlink의 대체재로서 적합한지 평가하기 위한 성능 분석 결과를 제공합니다.*
