...
def resolve_linux_paths():
    ...
    possible_paths = [
        ...
        os.path.join(ROOT_DIR, "build", "benchwithzlink-baseline"),
        os.path.join(ROOT_DIR, "build", "linux-x64", "benchwithzlink-baseline"),
    ]
    build_dir = next((p for p in possible_paths if os.path.exists(p)), possible_paths[0])
    libzlink_lib_dir = os.path.abspath(
        os.path.join(ROOT_DIR, "benchwithzlink-baseline", "libzlink", "libzlink_dist", "lib")
    )
    env_libzlink_dir = os.environ.get("BENCH_LIBZLINK_LIB_DIR")
    if env_libzlink_dir:
        libzlink_lib_dir = os.path.abspath(env_libzlink_dir)
    zlink_lib_dir = os.path.abspath(os.path.join(ROOT_DIR, "build", "bench", "lib"))
    return build_dir, libzlink_lib_dir, zlink_lib_dir
...
if IS_WINDOWS:
    ...
    LIBZLINK_LIB_DIR = os.path.abspath(os.path.join("benchwithzlink-baseline", "libzlink", "libzlink_dist", "bin"))
    env_libzlink_dir = os.environ.get("BENCH_LIBZLINK_BIN_DIR")
    if env_libzlink_dir:
        LIBZLINK_LIB_DIR = os.path.abspath(env_libzlink_dir)
    ZLINK_LIB_DIR = os.path.abspath(os.path.join("build", "windows-x64", "bin", "Release"))
else:
    BUILD_DIR, LIBZLINK_LIB_DIR, ZLINK_LIB_DIR = resolve_linux_paths()

DEFAULT_NUM_RUNS = 3  # Reduced from 10 for faster testing
CACHE_FILE = os.path.join(ROOT_DIR, "benchwithzlink-baseline", "libzlink_cache.json")
...
def get_env_for_lib(lib_name):
    env = base_env.copy()
    if IS_WINDOWS:
        env["PATH"] = f"{LIBZLINK_LIB_DIR};{env.get('PATH', '')}"
    else:
        if lib_name == "zlink":
...
def parse_args():
    usage = (
        ...
        "Options:\n"
        "  --refresh-libzlink        Refresh libzlink baseline cache\n"
...
def main():
    ...
    comparisons = [
        ...
        ("comp_std_zlink_dealer_dealer", "comp_zlink_dealer_dealer", "DEALER_DEALER"),
        ("comp_std_zlink_dealer_router", "comp_zlink_dealer_router", "DEALER_ROUTER"),
        ("comp_std_zlink_router_router", "comp_zlink_router_router", "ROUTER_ROUTER"),
        ("comp_std_zlink_router_router_poll", "comp_zlink_router_router_poll",
         "ROUTER_ROUTER_POLL"),
    ]
    ...
    for std_bin, zlk_bin, p_name in comparisons:
        ...
        if zlink_only:
            ...
            if refresh or p_name not in cache:
                s_stats, failures = collect_data(std_bin, "libzlink", p_name, num_runs)
                all_failures.extend(failures)
                cache[p_name] = s_stats
                with open(CACHE_FILE, 'w') as f: json.dump(cache, f, indent=2)
            else:
                print(f"  [libzlink] Using cached baseline.")
        ...
        for tr in TRANSPORTS:
            ...
            if zlink_only:
                ...
                print(
                    f"| {'Size':<{size_w}} | {'Metric':<{metric_w}} | {'Standard libzlink':>{val_w}} | {'zlink':>{val_w}} | {'Diff (%)':>{diff_w}} |"
                )
                print(
