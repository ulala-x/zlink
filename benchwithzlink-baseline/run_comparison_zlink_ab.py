...
def parse_args():
    usage = (
        "Usage: run_comparison_zlink_ab.py [PATTERN] [options]\n\n"
        "Options:\n"
...
def main():
    ...
    check_bin = os.path.join(rc.BUILD_DIR, "comp_std_zlink_pair" + rc.EXE_SUFFIX)
    ...
    comparisons = [
        ("comp_std_zlink_pair", "PAIR"),
        ("comp_std_zlink_pubsub", "PUBSUB"),
        ("comp_std_zlink_dealer_dealer", "DEALER_DEALER"),
        ("comp_std_zlink_dealer_router", "DEALER_ROUTER"),
        ("comp_std_zlink_router_router", "ROUTER_ROUTER"),
        ("comp_std_zlink_router_router_poll", "ROUTER_ROUTER_POLL"),
    ]
    ...
    for std_bin, p_name in comparisons:
        ...
        for tr in rc.TRANSPORTS:
            ...
            diff_w = 9
            print(
                f"| {'Size':<{size_w}} | {'Metric':<{metric_w}} | {'libzlink A':>{val_w}} | {'libzlink B':>{val_w}} | {'Diff (%)':>{diff_w}} |"
            )
            print(
            ...
            for sz in rc.MSG_SIZES:
                ...
                for _ in range(num_runs):
                    a_res = rc.run_single_test(std_bin, "libzlink", tr, sz)
                    if a_res is None:
                        all_failures.append((p_name, "libzlink", tr, sz, "timeout"))
                        continue
                    if not a_res:
                        all_failures.append((p_name, "libzlink", tr, sz, "no_data"))
                        continue
                    ...
                    b_res = rc.run_single_test(std_bin, "libzlink", tr, sz)
                    if b_res is None:
                        all_failures.append((p_name, "libzlink", tr, sz, "timeout"))
                        continue
                    if not b_res:
