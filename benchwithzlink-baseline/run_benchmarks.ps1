...
function Show-Usage {
    Write-Host @"
Usage: benchwithzlink-baseline\run_benchmarks.ps1 [options]

IMPORTANT: Use PowerShell parameter syntax with '-', not bash '--' syntax.

Options:
  -Help                Show this help.
  -SkipLibzlink         Skip libzlink baseline run (uses existing cache).
  -WithLibzlink         Run libzlink baseline and refresh cache (default).
  ...
  -ZlinkOnly          Run only zlink benchmarks (no libzlink baseline).
...
Examples:
  .\benchwithzlink-baseline\run_benchmarks.ps1
  .\benchwithzlink-baseline\run_benchmarks.ps1 -Pattern PAIR
  .\benchwithzlink-baseline\run_benchmarks.ps1 -Pattern DEALER_DEALER -Runs 5
  .\benchwithzlink-baseline\run_benchmarks.ps1 -Pattern ALL -Baseline
  .\benchwithzlink-baseline\run_benchmarks.ps1 -Runs 10 -Baseline
"@
}
...
# Build run command
$RunScript = Join-Path (Join-Path $RootDir "benchwithzlink-baseline") "run_comparison.py"
...
if ($ZlinkOnly) {
    ...
    $WithLibzlinkFlag = -not $SkipLibzlink
    if ($WithLibzlink) {
        $WithLibzlinkFlag = $true
    }

    if ($WithLibzlinkFlag) {
        $RunArgs += "--refresh-libzlink"
    } else {
        $CacheFile = Join-Path $RootDir "benchwithzlink-baseline" "libzlink_cache.json"
        if (-not (Test-Path $CacheFile)) {
            Write-Error "libzlink cache not found: $CacheFile"
            Write-Error "Run with -WithLibzlink once to generate the baseline."
            exit 1
        }
    }
}
