...
def parse_args():
    usage = (
        "Usage: run_comparison_zlink.py [PATTERN] [options]\n\n"
        "Options:\n"
...
def main():
    ...
    for std_bin, p_name in comparisons:
        ...
        print(f"\n## PATTERN: {p_name}")
        a_stats, failures = rc.collect_data(std_bin, "libzlink", p_name, num_runs)
        all_failures.extend(failures)
        b_stats, failures = rc.collect_data(std_bin, "libzlink", p_name, num_runs)
        all_failures.extend(failures)
        ...
        for tr in rc.TRANSPORTS:
            print(f"\n### Transport: {tr}")
            print(
                f"| {'Size':<{size_w}} | {'Metric':<{metric_w}} | {'libzlink A':>{val_w}} | {'libzlink B':>{val_w}} | {'Diff (%)':>{diff_w}} | {'Delta':>{val_w}} |"
            )
            print(
