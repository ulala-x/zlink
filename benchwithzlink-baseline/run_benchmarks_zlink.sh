...
usage() {
  cat <<'USAGE'
Usage: benchwithzlink-baseline/run_benchmarks_zlink.sh [options]
...
RUN_CMD=("${PYTHON_BIN[@]}" "${ROOT_DIR}/benchwithzlink-baseline/run_comparison_zlink.py" "${PATTERN}" --build-dir "${BUILD_DIR}" --runs "${RUNS}")
RUN_ENV=()
