...
1) Provider가 `zlink_provider_register()` 호출  
...
1) 클라이언트가 `zlink_gateway_send()` 호출  
...
| 단계 | 호출 | 설명 |
|------|------|------|
| 1 | `zlink_provider_new()` | Provider 생성 |
| 2 | `zlink_provider_bind()` | 비즈니스 ROUTER bind |
| 3 | `zlink_provider_connect_registry()` | Registry ROUTER 연결 |
| 4 | `zlink_provider_register()` | 서비스 등록 + Heartbeat 시작 |
| 5 | `zlink_provider_register_result()` | 등록 결과 확인(선택) |
| 6 | `zlink_provider_threadsafe_router()` | 비즈니스 ROUTER 소켓(thread-safe) 획득 |
| 7 | `zlink_provider_unregister()` | 종료 전 해제(선택) |
| 8 | `zlink_provider_destroy()` | 리소스 정리 |

#### Client (Discovery + Gateway)

| 단계 | 호출 | 설명 |
|------|------|------|
| 1 | `zlink_discovery_new()` | Discovery 생성 |
| 2 | `zlink_discovery_connect_registry()` | Registry PUB 연결 |
| 3 | `zlink_discovery_subscribe()` | 관심 서비스 등록 |
| 4 | `zlink_gateway_new()` | Gateway 생성 |
| 5 | `zlink_gateway_set_lb_strategy()` | 로드밸런싱 설정(선택) |
| 6 | `zlink_gateway_send()` | 메시지 전송 |
| 7 | `zlink_gateway_recv()` | 응답 수신 |
| 8 | `zlink_gateway_destroy()` | Gateway 정리 |
| 9 | `zlink_discovery_destroy()` | Discovery 정리 |
...
| 4 | `zlink_spot_node_register()` | **service_name=spot-node** 등록 |
| 5 | `zlink_discovery_new()` | Discovery 생성 |
| 6 | `zlink_discovery_connect_registry()` | Registry PUB 구독 |
| 7 | `zlink_discovery_subscribe()` | `spot-node` 구독 |
...
| 단계 | 호출 | 설명 |
|------|------|------|
| 1 | `zlink_registry_new()` | Registry 생성 |
| 2 | `zlink_registry_set_endpoints()` | PUB/ROUTER 설정 |
| 3 | `zlink_registry_add_peer()` | 클러스터 피어 추가(선택) |
| 4 | `zlink_registry_set_heartbeat()` | Heartbeat 정책 설정(선택) |
| 5 | `zlink_registry_set_broadcast_interval()` | 브로드캐스트 주기 설정(선택) |
| 6 | `zlink_registry_start()` | Registry 시작 |
| 7 | `zlink_registry_destroy()` | 종료 |
...
struct service_entry_t {
    ...
    zlink_routing_id_t routing_id;    // Provider routing_id
...
- `zlink_discovery_subscribe()` / `zlink_discovery_unsubscribe()`는
...
- **커스텀 로드밸런싱**이 필요하면:
  ...
  - `zlink_gateway_threadsafe_router()`로 ROUTER 소켓을 얻어
...
- 실사용에서는 다음 흐름이 **Gateway의 Request/Reply API**로 간주된다.
  - `zlink_gateway_send()` → `request_id` 획득
  - `zlink_gateway_recv()` → `request_id` 기반 매핑
- 타임아웃/재시도 정책은 애플리케이션 레벨에서 관리한다.
- 03번 문서의 `zlink_request_send/recv`와 **네이밍을 맞춘 별칭 API**를 제공할 수 있다.
...
2. 비즈니스 소켓 bind
   └─► zlink_provider_bind("tcp://*:5555")
   └─► (선택) ZLINK_ROUTING_ID 설정으로 안정적 routing_id 사용
   └─► ROUTER 소켓 생성 및 bind

3. Registry 연결
   └─► zlink_provider_connect_registry()
   └─► DEALER connect to Registry

4. 서비스 등록
   └─► zlink_provider_register()
   └─► REGISTER 메시지 전송
...
6. 종료 시
   └─► zlink_provider_unregister() 또는 zlink_provider_destroy()
   └─► UNREGISTER 메시지 전송
```
...
- **bind_endpoint**: ROUTER 소켓을 bind할 주소 (`zlink_provider_bind`)
- **advertise_endpoint**: Registry에 등록할 주소 (`zlink_provider_register`)
...
- routing_id를 안정적으로 유지하려면 Provider가 비즈니스 소켓에
  `ZLINK_ROUTING_ID`를 명시적으로 설정해야 한다.
- weight 변경은 `zlink_provider_update_weight()` 또는 동일 키 재등록으로 갱신한다.
...
/* Registry 설정 */
ZLINK_EXPORT int zlink_registry_set_endpoints(
    void *registry,
...
/* Registry ID 설정 (선택) */
ZLINK_EXPORT int zlink_registry_set_id(
    void *registry,
    uint32_t registry_id
);

/* 클러스터 피어 추가 */
ZLINK_EXPORT int zlink_registry_add_peer(
    void *registry,
    const char *peer_pub_endpoint  // 다른 Registry의 PUB endpoint
);

/* Heartbeat 설정 */
ZLINK_EXPORT int zlink_registry_set_heartbeat(
    void *registry,
...
/* 브로드캐스트 주기 설정 */
ZLINK_EXPORT int zlink_registry_set_broadcast_interval(
    void *registry,
...
/* 서비스 구독 (로컬 필터 등록) */
ZLINK_EXPORT int zlink_discovery_subscribe(
    void *discovery,
    const char *service_name
);

/* 서비스 구독 해제 (로컬 필터 해제) */
ZLINK_EXPORT int zlink_discovery_unsubscribe(
    void *discovery,
    const char *service_name
);

/* Provider 목록 조회 */
ZLINK_EXPORT int zlink_discovery_get_providers(
    void *discovery,
    const char *service_name,
...
/* Provider 수 조회 */
ZLINK_EXPORT int zlink_discovery_provider_count(
    void *discovery,
    const char *service_name
);

/* 서비스 가용 여부 */
ZLINK_EXPORT int zlink_discovery_service_available(
    void *discovery,
...
/* 메시지 수신 (multipart) */
ZLINK_EXPORT int zlink_gateway_recv(
    void *gateway,
    zlink_msg_t **parts,
    size_t *part_count,
    int flags,
 ...
 * @param reply_parts  응답 메시지 배열 (필요 시 zlink_msgv_close)
 * @param reply_count  응답 프레임 개수
 * @param error        0: 성공 (그 외 errno)
 */
typedef void (*zlink_gateway_request_cb_fn)(
    uint64_t request_id,
...
/*
 * zlink_gateway_request - 요청 전송 (콜백 기반, 내부 LB)
 *
 ...
 *                    - ZLINK_REQUEST_TIMEOUT_DEFAULT: 기본값 사용
 *                    - -1: 무제한
 * @return             요청 ID (>0), 실패 시 0
 *
 * 참고:
 *   - reply_parts는 콜백 반환 전까지 유효, 사용 후 zlink_msgv_close 필요
 *   - 에러 시 reply_parts=NULL, reply_count=0
 */
ZLINK_EXPORT uint64_t zlink_gateway_request(
    void *gateway,
    const char *service_name,
    zlink_msg_t *parts,
    size_t part_count,
...
/*
 * zlink_gateway_request_send - 요청 전송 (내부 LB)
 * - 반환: request_id (>0), 실패 시 0
 */
ZLINK_EXPORT uint64_t zlink_gateway_request_send(
    void *gateway,
    const char *service_name,
    zlink_msg_t *parts,
    size_t part_count,
...
/*
 * zlink_gateway_request_recv - 완료된 응답 수신
 * - 내부적으로 zlink_gateway_recv를 사용
 * - 성공 시 completion->parts를 zlink_msgv_close로 정리 필요
 * - zlink_gateway_threadsafe_router()으로 직접 송신한 요청은
 *   completion.service_name을 보장하지 않는다
 */
ZLINK_EXPORT int zlink_gateway_request_recv(
    void *gateway,
...
/* 연결된 Provider 수 조회 */
ZLINK_EXPORT int zlink_gateway_connection_count(
    void *gateway,
    const char *service_name
);
...
/* 가중치 갱신 (등록된 서비스 대상) */
ZLINK_EXPORT int zlink_provider_update_weight(
    void *provider,
    const char *service_name,
...
/* 서비스 해제 */
ZLINK_EXPORT int zlink_provider_unregister(
    void *provider,
    const char *service_name
);

/* 등록 결과 확인 (비동기 등록 후) */
ZLINK_EXPORT int zlink_provider_register_result(
    void *provider,
    const char *service_name,
...
// 비즈니스 소켓 bind (Gateway 연결 수신용)
zlink_provider_bind(provider, "tcp://*:5555");

// Registry 연결
zlink_provider_connect_registry(provider, "tcp://registry1:5551");

// 서비스 등록 (advertise_endpoint 자동 감지)
zlink_provider_register(provider,
    "payment-service",
...
char error_msg[256];
zlink_provider_register_result(provider, "payment-service",
...
// 비즈니스 소켓으로 메시지 처리
void *socket = zlink_provider_threadsafe_router(provider);

while (running) {
    zlink_msg_t routing_id_frame, request_id_frame, msgid_frame;
    zlink_msg_init(&routing_id_frame);
    zlink_msg_init(&request_id_frame);
    zlink_msg_init(&msgid_frame);

    // ROUTER 소켓: [routing_id][request_id][msgId][payload...]
    zlink_msg_recv(&routing_id_frame, socket, 0);
    zlink_msg_recv(&request_id_frame, socket, 0);
    zlink_msg_recv(&msgid_frame, socket, 0);

    uint64_t request_id = *(uint64_t*)zlink_msg_data(&request_id_frame);
    uint16_t msgId = *(uint16_t*)zlink_msg_data(&msgid_frame);

    // dispatch
    zlink_msg_t response;
    zlink_msg_init(&response);
    // payload는 multipart 가능: 필요한 만큼 누적 처리
    while (1) {
        zlink_msg_t part;
        zlink_msg_init(&part);
        zlink_msg_recv(&part, socket, 0);
        bool more = zlink_msg_more(&part);
    ...
    zlink_msg_send(&routing_id_frame, socket, ZLINK_SNDMORE);
    zlink_msg_send(&request_id_frame, socket, ZLINK_SNDMORE);
    zlink_msg_send(&response, socket, 0);
    zlink_msg_close(&response);

    zlink_msg_close(&routing_id_frame);
    zlink_msg_close(&request_id_frame);
    zlink_msg_close(&msgid_frame);
}
...
zlink_discovery_subscribe(discovery, "user-service");

// ===== Gateway 설정 (메시지 라우팅) =====
void *gateway = zlink_gateway_new(ctx, discovery);

// 로드밸런싱 전략 설정
zlink_gateway_set_lb_strategy(gateway, "payment-service", ZLINK_GATEWAY_LB_ROUND_ROBIN);
zlink_gateway_set_lb_strategy(gateway, "user-service", ZLINK_GATEWAY_LB_WEIGHTED);
...
// 메시지 전송 (자동 로드밸런싱)
zlink_msg_t request;
zlink_msg_init_data(&request, data, size, NULL, NULL);

uint64_t request_id;
zlink_gateway_send(gateway, "payment-service", &request, 1, 0, &request_id);
...
zlink_gateway_recv(gateway, &reply_parts, &reply_count, 0, service_name, &recv_request_id);
...
void *discovery = zlink_discovery_new(ctx);
zlink_discovery_connect_registry(discovery, "tcp://registry:5550");
zlink_discovery_subscribe(discovery, "payment-service");

// Provider 목록 조회
zlink_provider_info_t providers[10];
size_t count = 10;
zlink_discovery_get_providers(discovery, "payment-service", providers, &count);
...
zlink_provider_info_t providers[16];
size_t count = 16;
zlink_discovery_get_providers(discovery, "payment-service", providers, &count);
...
void *router = zlink_gateway_threadsafe_router(gateway);

// 프레임: [routing_id][request_id][msgId][payload...]
uint64_t request_id = next_request_id(); // 앱에서 생성
uint16_t msgId = MSG_PAYMENT_REQUEST;

zlink_msg_t rid, reqid, msgid, payload;
zlink_msg_init_size(&rid, providers[idx].routing_id.size);
memcpy(zlink_msg_data(&rid), providers[idx].routing_id.data, providers[idx].routing_id.size);

zlink_msg_init_size(&reqid, sizeof(uint64_t));
memcpy(zlink_msg_data(&reqid), &request_id, sizeof(uint64_t));

zlink_msg_init_size(&msgid, sizeof(uint16_t));
memcpy(zlink_msg_data(&msgid), &msgId, sizeof(uint16_t));

zlink_msg_init_data(&payload, data, size, NULL, NULL);

zlink_msg_send(&rid, router, ZLINK_SNDMORE);
zlink_msg_send(&reqid, router, ZLINK_SNDMORE);
zlink_msg_send(&msgid, router, ZLINK_SNDMORE);
zlink_msg_send(&payload, router, 0);
...
// 두 서비스에 동시에 요청 전송
zlink_msg_t pay_req;
zlink_msg_init_data(&pay_req, pay_data, pay_size, NULL, NULL);
uint64_t pay_request_id = 0;
zlink_gateway_send(gateway, "payment-service", &pay_req, 1, 0, &pay_request_id);

zlink_msg_t user_req;
zlink_msg_init_data(&user_req, user_data, user_size, NULL, NULL);
uint64_t user_request_id = 0;
zlink_gateway_send(gateway, "user-service", &user_req, 1, 0, &user_request_id);
...
while (pending > 0) {
    ...
    if (zlink_gateway_recv(gateway, &reply_parts, &reply_count, 0,
...
uint64_t req_id = zlink_gateway_request(
    gateway,
    "payment-service",
    &req,
    1,
...
zlink_spot_node_register(node, "spot-node", NULL);   // advertise는 PUB endpoint
zlink_spot_node_set_discovery(node, discovery, "spot-node");

// SPOT 인스턴스 사용
void *spot = zlink_spot_new(node);
zlink_spot_subscribe(spot, "metrics:zone1:*");
```
...
| 파일 | 변경 내용 |
|------|----------|
| `include/zlink.h` | Discovery, Gateway, Provider API 선언 |
| `src/api/zlink.cpp` | API 구현 |
...
- `test_gateway_router_mixing_guard`:
  - `zlink_gateway_threadsafe_router()`로 직접 송신한 요청의 응답은
...
2. `zlink_provider_update_weight()` 호출 (예: weight=5)
...
1. Gateway에서 `zlink_gateway_request()` 호출
2. Provider가 응답 전송
3. 콜백이 호출되고 request_id 매핑 확인

시나리오 8: 커스텀 라우팅 (ROUTER 직접 사용)
1. Discovery에서 Provider 목록 조회
2. `zlink_gateway_threadsafe_router()`로 ROUTER 소켓 획득
...
| 0.4 | 2026-01-26 | Provider bind 명확화: zlink_provider_bind() API 추가, bind_endpoint/advertise_endpoint 구분, 예제 코드 수정 |
