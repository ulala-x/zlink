...
```c
int zlink_socket_monitor(void *socket, const char *endpoint, int events);
```

- 호출 시 내부 모니터 소켓이 `endpoint`에 bind
- 사용자는 별도의 ZLINK_PAIR 소켓을 만들어 `endpoint`에 connect 후 이벤트 수신
...
2. 모니터 활성화
   ...
   - 또는 zlink_socket_monitor_open으로 자동 생성/연결

3. 핸드셰이크
   - 양쪽이 routing_id(가변 길이) 전송
   - 양쪽이 상대방 routing_id 저장
   - 핸드셰이크 완료 시 `ZLINK_EVENT_CONNECTION_READY` 발생

4. 모니터링 이벤트 수신
   - 사용자는 monitor socket에서 zlink_monitor_recv 호출
   - 다중 소켓 모니터링은 zlink_poll로 조합
```

### 2.3 연결 완료 이벤트 명확화

- **연결 완료 기준**을 *핸드셰이크 완료 시점*으로 정의한다.
- 이벤트 명칭은 `ZLINK_EVENT_CONNECTION_READY`로 통일한다.
...
- 드롭 수는 `zlink_socket_stats_t.monitor_events_dropped`로 노출한다.
...
```c
#define ZLINK_EVENT_CONNECTION_READY 0x1000 /* HANDSHAKE_SUCCEEDED 대체 */
```
...
typedef struct {
    ...
    zlink_routing_id_t routing_id; // 상대방 routing_id (size=0이면 없음)
    char     local_addr[256]; // 로컬 주소
    char     remote_addr[256];// 원격 주소
} zlink_monitor_event_t;
```

- 연결 완료 이벤트는 `ZLINK_EVENT_CONNECTION_READY` 사용 (핸드셰이크 완료 시 발생)
...
ZLINK_EXPORT int zlink_socket_monitor(
    void *socket,
    const char *endpoint,
    int events
);
...
```c
/*
 * zlink_socket_monitor_open - 모니터 소켓 자동 생성
 *
 ...
 *   - `ZLINK_EVENT_MONITOR_STOPPED`는 종료 시 마지막 이벤트로 전달 가능
 *   - monitor_open은 `zlink_socket_monitor(...)`의 간편 래퍼로 간주
 *
 * 종료:
 *   - 모니터 종료 시 zlink_socket_monitor(socket, NULL, 0) 호출 권장
 *   - 반환된 monitor_socket은 사용 후 zlink_close로 정리
 */
...
```c
/*
 * zlink_monitor_recv - 모니터링 이벤트 수신
 *
 * @param monitor_socket  모니터링 수신용 PAIR 소켓
 * @param event           수신된 이벤트 (out)
 * @param flags           0 또는 ZLINK_DONTWAIT
 * @return                0: 성공, -1: 에러 또는 타임아웃
 *
 * 에러:
 *   EAGAIN    - non-blocking 모드에서 이벤트 없음
 *   ETIMEDOUT - 타임아웃 만료 (zlink_poll로 제어 시)
 *
 * 참고:
 *   - 다중 소켓 모니터링은 zlink_poll로 조합
 *   - ZLINK_DONTWAIT는 즉시 반환
 *   - 단일 포맷 이벤트를 파싱하여 zlink_monitor_event_t로 변환
 */
ZLINK_EXPORT int zlink_monitor_recv(
    void *monitor_socket,
...
`ZLINK_EVENT_DISCONNECTED`는 **의도/비의도와 무관하게 항상 발생**한다.  
...
|------|------|------|
| 0 | UNKNOWN | 원인 불명 |
| 1 | LOCAL | 로컬에서 `zlink_disconnect()` 또는 `zlink_close()` 호출로 의도적 종료 |
...
```c
typedef struct {
    zlink_routing_id_t routing_id; // 피어 routing_id
    char     remote_addr[256];   // 원격 주소
...
ZLINK_EXPORT int zlink_socket_peer_info(
    void *socket,
    const zlink_routing_id_t *routing_id,
    zlink_peer_info_t *info
);

ZLINK_EXPORT int zlink_socket_peer_routing_id(
    void *socket,
    int index,
    zlink_routing_id_t *out
);

ZLINK_EXPORT int zlink_socket_peer_count(void *socket);

ZLINK_EXPORT int zlink_socket_peers(
    void *socket,
...
```c
void *sock_a = zlink_socket(ctx, ZLINK_ROUTER);
void *sock_b = zlink_socket(ctx, ZLINK_DEALER);
/* ... bind/connect 생략 ... */
...
if (rc > 0) {
    if (items[0].revents & ZLINK_POLLIN) {
        zlink_monitor_event_t ev;
        if (zlink_monitor_recv(mon_a, &ev, ZLINK_DONTWAIT) == 0) {
            /* handle ev */
        }
    }
    if (items[1].revents & ZLINK_POLLIN) {
        zlink_monitor_event_t ev;
        if (zlink_monitor_recv(mon_b, &ev, ZLINK_DONTWAIT) == 0) {
            /* handle ev */
        }
    }
}
...
| 파일 | 변경 내용 |
|------|----------|
| `include/zlink.h` | `ZLINK_EVENT_CONNECTION_READY`, `zlink_socket_monitor_open`, `zlink_monitor_recv` 선언 |
| `src/api/zlink.cpp` | monitor_open 구현 + recv 파서 구현 |
