...
static void server_echo_handler (zlink_msg_t *parts,
                                 size_t part_count,
                                 ...
                                 uint64_t request_id)
{
    ...
    const int rc = zlink_reply (g_server, routing_id, request_id, &reply, 1);
    g_reply_rc.store (rc);
    g_reply_errno.store (rc == 0 ? 0 : errno);
}

static void server_no_reply_handler (zlink_msg_t *parts,
                                     size_t part_count,
...
static void server_group_handler (zlink_msg_t *parts,
                                  size_t part_count,
                                  const zlink_routing_id_t *routing_id,
                                  uint64_t request_id)
{
    ...
    zlink_msg_t reply;
    zlink_msg_init_size (&reply, 2);
    memcpy (zlink_msg_data (&reply), "OK", 2);
    zlink_reply (g_server, routing_id, request_id, &reply, 1);
}

static void server_reply_simple_handler (zlink_msg_t *parts,
                                         size_t part_count,
                                         const zlink_routing_id_t *,
                                         uint64_t)
{
    ...
    zlink_msg_t reply;
    zlink_msg_init_size (&reply, 6);
    memcpy (zlink_msg_data (&reply), "SIMPLE", 6);
    const int rc = zlink_reply_simple (g_server, &reply, 1);
    g_reply_rc.store (rc);
    g_reply_errno.store (rc == 0 ? 0 : errno);
}

static void server_multipart_handler (zlink_msg_t *parts,
                                      size_t part_count,
                                      const zlink_routing_id_t *routing_id,
                                      uint64_t request_id)
{
    ...
    if (parts && part_count == 2) {
        const char *head = static_cast<const char *> (zlink_msg_data (&parts[0]));
        const size_t head_size = zlink_msg_size (&parts[0]);
        const char *body = static_cast<const char *> (zlink_msg_data (&parts[1]));
        const size_t body_size = zlink_msg_size (&parts[1]);
    ...
    zlink_msg_init_size (&reply_parts[0], 5);
    memcpy (zlink_msg_data (&reply_parts[0]), "resp1", 5);
    zlink_msg_init_size (&reply_parts[1], 5);
    memcpy (zlink_msg_data (&reply_parts[1]), "resp2", 5);
    const int rc = zlink_reply (g_server, routing_id, request_id, reply_parts, 2);
    if (rc != 0) {
...
static void server_pipeline_handler (zlink_msg_t *parts,
                                     size_t part_count,
                                     const zlink_routing_id_t *routing_id,
                                     uint64_t request_id)
{
    ...
    if (static_cast<int> (g_pipeline_entries.size ()) == pipeline_count) {
        for (int i = pipeline_count - 1; i >= 0; --i) {
            ...
            zlink_msg_t reply;
            zlink_msg_init_size (&reply, static_cast<size_t> (len));
            if (len > 0)
                memcpy (zlink_msg_data (&reply), buf, static_cast<size_t> (len));
            zlink_reply (g_server, &item.routing_id, item.request_id, &reply, 1);
        }
        g_pipeline_entries.clear ();
    }
}
                             ...
                             int error)
{
    ...
    if (error == 0 && reply_parts && reply_count > 0) {
        g_reply_size = zlink_msg_size (&reply_parts[0]);
        if (g_reply_size > sizeof (g_reply_buf))
            g_reply_size = sizeof (g_reply_buf);
        memcpy (g_reply_buf, zlink_msg_data (&reply_parts[0]), g_reply_size);
    }
                                       ...
                                       int error)
{
    ...
    if (error == 0 && reply_parts && reply_count == 2) {
        const char *first =
          static_cast<const char *> (zlink_msg_data (&reply_parts[0]));
        const size_t first_size = zlink_msg_size (&reply_parts[0]);
        const char *second =
          static_cast<const char *> (zlink_msg_data (&reply_parts[1]));
        const size_t second_size = zlink_msg_size (&reply_parts[1]);
    ...
    if (error == 0 && reply_parts && reply_count > 0) {
        ...
        if (index >= 0) {
            ...
            if (len > 0
                ...
                && memcmp (zlink_msg_data (&reply_parts[0]), expected,
                           static_cast<size_t> (len))
                     == 0) {
                ok = true;
            }
...
static void test_callback_request_reply ()
{
    ...
    g_server = zlink_socket_threadsafe (ctx, ZLINK_ROUTER);
    TEST_ASSERT_NOT_NULL (g_server);

    void *client = zlink_socket_threadsafe (ctx, ZLINK_DEALER);
    TEST_ASSERT_NOT_NULL (client);

    TEST_ASSERT_SUCCESS_ERRNO (zlink_bind (g_server, ENDPOINT_0));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_connect (client, ENDPOINT_0));
    msleep (SETTLE_TIME);

    TEST_ASSERT_SUCCESS_ERRNO (zlink_on_request (g_server, server_echo_handler));

    int correlate = 0;
    size_t correlate_size = sizeof (correlate);
    TEST_ASSERT_SUCCESS_ERRNO (zlink_getsockopt (g_server, ZLINK_REQUEST_CORRELATE,
                                               &correlate, &correlate_size));
    TEST_ASSERT_EQUAL_INT (1, correlate);
    correlate = 0;
    correlate_size = sizeof (correlate);
    TEST_ASSERT_SUCCESS_ERRNO (zlink_getsockopt (client, ZLINK_REQUEST_CORRELATE,
                                               &correlate, &correlate_size));
    TEST_ASSERT_EQUAL_INT (1, correlate);

    zlink_msg_t request;
    zlink_msg_init_size (&request, 5);
    memcpy (zlink_msg_data (&request), "Hello", 5);

    uint64_t req_id = zlink_request (client, NULL, &request, 1, client_callback,
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_close (g_server));
...
static void test_polling_request_reply ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_bind (g_server, ENDPOINT_1));
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_request_recv (client, &completion, 2000));
    ...
    TEST_ASSERT_EQUAL_MEMORY ("World",
                              zlink_msg_data (&completion.parts[0]),
                              zlink_msg_size (&completion.parts[0]));
...
static void test_request_timeout ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_bind (g_server, ENDPOINT_2));
    TEST_ASSERT_SUCCESS_ERRNO (zlink_connect (client, ENDPOINT_2));
    msleep (SETTLE_TIME);

    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_on_request (g_server, server_no_reply_handler));

    zlink_msg_t request;
    zlink_msg_init_size (&request, 4);
    memcpy (zlink_msg_data (&request), "Wait", 4);

    uint64_t req_id =
      zlink_request (client, NULL, &request, 1, client_callback, 50);
    TEST_ASSERT_TRUE (req_id > 0);
...
static void test_router_router_targeted ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_setsockopt (g_server, ZLINK_ROUTING_ID, server_id,
                      strlen (server_id)));
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_setsockopt (client, ZLINK_ROUTING_ID, client_id,
                      strlen (client_id)));

    TEST_ASSERT_SUCCESS_ERRNO (zlink_bind (g_server, ENDPOINT_3));
    ...
    uint64_t req_id = zlink_request (client, &target, &request, 1,
                                   client_callback,
                                   ZLINK_REQUEST_TIMEOUT_DEFAULT);
    TEST_ASSERT_TRUE (req_id > 0);
...
static void test_multipart_request_reply ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_on_request (g_server, server_multipart_handler));

    zlink_msg_t parts[2];
    zlink_msg_init_size (&parts[0], 4);
    memcpy (zlink_msg_data (&parts[0]), "head", 4);
    zlink_msg_init_size (&parts[1], 4);
    memcpy (zlink_msg_data (&parts[1]), "body", 4);

    uint64_t req_id = zlink_request (client, NULL, parts, 2,
...
static void test_group_request_order ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zlink_on_request (g_server, server_group_handler));

    zlink_msg_t msg1;
    zlink_msg_init_size (&msg1, 1);
    memcpy (zlink_msg_data (&msg1), "A", 1);
    uint64_t req1 =
      zlink_group_request (client, NULL, 42, &msg1, 1, client_group_callback,
                         2000);
    TEST_ASSERT_TRUE (req1 > 0);

    zlink_msg_t msg2;
    zlink_msg_init_size (&msg2, 1);
    memcpy (zlink_msg_data (&msg2), "B", 1);
    uint64_t req2 =
      zlink_group_request (client, NULL, 42, &msg2, 1, client_group_callback,
                         2000);
...
static void test_pipeline_requests ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_on_request (g_server, server_pipeline_handler));
    ...
    for (int i = 0; i < pipeline_count; ++i) {
        ...
        if (len < 0)
            len = 0;
        zlink_msg_t request;
        zlink_msg_init_size (&request, static_cast<size_t> (len));
        if (len > 0)
            memcpy (zlink_msg_data (&request), buf, static_cast<size_t> (len));
...
static void test_cancel_all_requests ()
{
    ...
    memcpy (zlink_msg_data (&request), "Wait", 4);
    uint64_t req_id =
      zlink_request (client, NULL, &request, 1, client_callback, 5000);
    TEST_ASSERT_TRUE (req_id > 0);

    msleep (10);
    TEST_ASSERT_EQUAL_INT (1, zlink_pending_requests (client));

    TEST_ASSERT_EQUAL_INT (1, zlink_cancel_all_requests (client));

    wait_for (g_client_called, 2000);
    TEST_ASSERT_TRUE (g_client_called.load ());
    TEST_ASSERT_EQUAL_INT (ECANCELED_ZLINK, g_client_error.load ());
    TEST_ASSERT_EQUAL_INT (0, zlink_pending_requests (client));
...
static void test_reply_simple ()
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zlink_on_request (g_server, server_reply_simple_handler));
