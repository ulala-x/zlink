...
static void assert_auto_routing_id (void *socket_)
{
    ...
    TEST_ASSERT_SUCCESS_ERRNO (
      zmq_getsockopt (socket_, ZMQ_ROUTING_ID, buf, &size));
...
static bool wait_for_event (void *monitor_,
                            ...
                            zmq_monitor_event_t *out_)
{
    for (int attempt = 0; attempt < 50; ++attempt) {
        zmq_pollitem_t items[] = {{monitor_, 0, ZMQ_POLLIN, 0}};
        const int rc = zmq_poll (items, 1, 200);
        if (rc > 0 && (items[0].revents & ZMQ_POLLIN)) {
            zmq_monitor_event_t ev;
            while (zmq_monitor_recv (monitor_, &ev, ZMQ_DONTWAIT) == 0) {
...
{
    const int types[] = {ZMQ_PAIR,   ZMQ_PUB,   ZMQ_SUB, ZMQ_DEALER,
                         ZMQ_ROUTER, ZMQ_XPUB, ZMQ_XSUB, ZMQ_STREAM};
...
void test_monitor_open_and_connection_ready ()
{
    ...
    void *mon = zmq_socket_monitor_open (server,
    ...
    zmq_monitor_event_t ev;
    TEST_ASSERT_TRUE (
      wait_for_event (mon, ZMQ_EVENT_CONNECTION_READY, &ev));
    ...
    TEST_ASSERT_TRUE (wait_for_event (mon, ZMQ_EVENT_DISCONNECTED, NULL));
...
void test_metrics_and_peer_enumeration ()
{
    ...
    void *mon = zmq_socket_monitor_open (server, ZMQ_EVENT_CONNECTION_READY);
    TEST_ASSERT_NOT_NULL (mon);

    TEST_ASSERT_SUCCESS_ERRNO (zmq_connect (client, endpoint));
    TEST_ASSERT_TRUE (wait_for_event (mon, ZMQ_EVENT_CONNECTION_READY, NULL));

    const int peer_count = zmq_socket_peer_count (server);
    TEST_ASSERT_TRUE (peer_count >= 1);

    zmq_routing_id_t peer_id;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_peer_routing_id (server, 0, &peer_id));
    ...
    int rid_size = zmq_recv (server, rid_buf, sizeof (rid_buf), 0);
    ...
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_peer_info (server, &peer_id, &info));
    TEST_ASSERT_TRUE (info.routing_id.size > 0);

    TEST_ASSERT_EQUAL_INT (
      peer_id.size,
      TEST_ASSERT_SUCCESS_ERRNO (
        zmq_send (server, peer_id.data, peer_id.size, ZMQ_SNDMORE)));
    send_string_expect_success (server, payload, 0);
    recv_string_expect_success (client, payload, 0);

    zmq_socket_stats_t stats;
    TEST_ASSERT_SUCCESS_ERRNO (zmq_socket_stats (server, &stats));
