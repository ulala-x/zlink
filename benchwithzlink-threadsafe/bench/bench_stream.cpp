...
std::vector<unsigned char> expect_connect_event(void* socket) {
    unsigned char id_buf[255];
    int id_len = bench_recv_fast(socket, id_buf, sizeof(id_buf), 0, "stream id");
    if (id_len <= 0) {
    ...
    int payload_len = bench_recv_fast(socket, payload, sizeof(payload), 0, "stream event");
...
inline int recv_stream_msg(void* socket,
                           ...
                           size_t buf_size) {
    unsigned char id_buf[255];
    int id_len = bench_recv_fast(socket, id_buf, sizeof(id_buf), 0, "stream id recv");
    if (id_len <= 0) {
        return -1;
    }
    if (routing_id_out) {
        routing_id_out->assign(id_buf, id_buf + id_len);
    }
    return bench_recv_fast(socket, buf, buf_size, 0, "stream payload recv");
}

void run_stream(const std::string& transport, size_t msg_size, int msg_count, const std::string& lib_name) {
    void *ctx = zlink_ctx_new();
    void *server = bench_socket(ctx, ZLINK_STREAM);
    void *client = bench_socket(ctx, ZLINK_STREAM);
